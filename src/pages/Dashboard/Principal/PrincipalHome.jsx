...
export default function PrincipalHome() {
  ...
  useEffect(() => {
    try {
      ...
      console.error("Principal user data error:", error);
    }
  }, []);
  ...
  const fetchAPI = async (endpoint) => {
    ...
    if (result?.status === false) {
      throw new Error(
        result?.message || `${endpoint} failed`
      );
    }
    ...
    async (showLoader = false) => {
      try {
        ...
        if (results[0].status === "fulfilled") {
          setStudents(
            getArrayFromResponse(results[0].value)
          );
        } else {
          console.error(
        ...
        if (results[1].status === "fulfilled") {
          setTeachers(
            getArrayFromResponse(results[1].value)
          );
        } else {
          console.error(
        ...
        console.error(
          "Principal dashboard loading error:",
          error
        );
      } finally {
        setLoading(false);
  ...
  const todayAdmissions = useMemo(() => {
    ...
    return normalizedStudents.filter(
      (student) =>
        student.admissionDate &&
        String(student.admissionDate).slice(
          0,
          10
        ) === today
    ).length;
