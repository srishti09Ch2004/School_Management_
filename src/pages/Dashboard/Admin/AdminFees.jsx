...
//  Add New Fee
  ...
  const handleFeeSubmit = async () => {
    ...
    const paidFee = Number(feeForm.paid_fee);
    ...
    if (paidFee < 0) {
      alert("Paid fee cannot be negative");
      return;
    }
  ...
  const handlePaymentSubmit = async () => {
    ...
    const amount = Number(paymentForm.payment_amount);
    const due = Number(payFeeData?.due_fee || 0);
    ...
    try {
      const response = await fetch(
        "http://localhost/SCHOOL_MANAGEMENT_SYSTEM/backend/api/admin/addFeePayment.php",
        {
          ...
          body: JSON.stringify({
            fee_id: Number(payFeeData.id),
            student_id: Number(payFeeData.student_id),
            amount: amount,
            payment_date: paymentForm.payment_date,
            payment_method: paymentForm.payment_method,
